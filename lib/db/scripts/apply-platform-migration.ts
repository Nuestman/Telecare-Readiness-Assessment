...
async function renameSurveysTable(client: import("pg").PoolClient): Promise<void> {
  ...
  const surveysExists = rows[0]?.exists ?? false;
  if (!surveysExists) return;

  const { rows: targetRows } = await client.query<{ exists: boolean }>(`
    SELECT EXISTS (
      SELECT 1 FROM information_schema.tables
  ...
  if (targetRows[0]?.exists) {
    console.log("telehealth_readiness_surveys already exists; skipping rename");
    return;
  }

  await client.query(`ALTER TABLE surveys RENAME TO telehealth_readiness_surveys`);
  console.log("Renamed surveys → telehealth_readiness_surveys");
}

async function dropStudySlugColumn(client: import("pg").PoolClient): Promise<void> {
  const { rows } = await client.query<{ exists: boolean }>(`
    SELECT EXISTS (
  ...
  if (rows[0]?.exists) {
    ...
    console.log("Dropped study_slug from telehealth_readiness_surveys");
  }
}

async function seedStudies(client: import("pg").PoolClient): Promise<void> {
  await client.query(`
    INSERT INTO studies (
  ...
  `);
  console.log("Seeded studies registry");
}

async function backfillStudyAccess(client: import("pg").PoolClient): Promise<void> {
  await client.query(`
  ...
  `);
  console.log("Backfilled admin_user_study_access for telehealth-readiness");
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await renameSurveysTable(client);
    await dropStudySlugColumn(client);
    await seedStudies(client);
    await backfillStudyAccess(client);
