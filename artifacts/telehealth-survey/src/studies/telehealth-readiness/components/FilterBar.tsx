...
export type SurveyFilters = ListSurveysParams;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4 border rounded-lg bg-card">
      ...
      <div className="space-y-2">
        ...
        <Select
          ...
          onValueChange={(v) =>
            onChange({ ...filters, employment_type: v === 'all' ? undefined : v })
          }
        >
      ...
      <div className="space-y-2">
        <Label>NCD status</Label>
        <Select
          value={filters.has_ncd ?? 'all'}
          onValueChange={(v) =>
            onChange({ ...filters, has_ncd: v === 'all' ? undefined : v })
          }
        >
          <SelectTrigger><SelectValue placeholder="All" /></SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-2 lg:col-span-2">
        <Label htmlFor="work_area">Work area (contains)</Label>
        <Input
          id="work_area"
          placeholder="e.g. processing"
          value={filters.work_area ?? ''}
          onChange={(e) =>
            onChange({ ...filters, work_area: e.target.value || undefined })
          }
        />
      </div>
      <div className="space-y-2">
